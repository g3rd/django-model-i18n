...
def validate_fields(cls, model):
    ...
    if hasattr(cls, 'fields'):
        ...
        if len(cls.fields) > len(set(cls.fields)):
            raise ImproperlyConfigured('There are duplicate \
            field(s) in %s.fields' % cls.__name__)
    else:
        raise ImproperlyConfigured('%s.fields is a is a required \
